...
* emit a CST instead of an AST
* drastically improve error recovery
* perform Unicode NFC-normalization for identifiers (for ASTs) / provide a public API that does so (for CSTs)
* experiment with further improving certain parsing "primitives" by rendering them more "principled"
  * this concerns areas like "prefix matching" (esp. "qualifiers"), token jointness, lookahead & backtracking
* experiment with incremental parsing, maybe
